...
bedtowig <- function(pathToBed, tilesdt){
    
    tilesdt <- fread("../bins_500kb.csv")
    out <- gsub(".bed", ".wig", pathToBed)
    
    fragbed <- fread(pathToBed)
...
bin.genome <- function(binsize, build="hg19", chrX=TRUE) {
    ...
    data(filters.hg19, package="PlasmaTools", envir=environment())
...
getArms <- function(build="hg19", chrX=FALSE) {
    library(BSgenome.Hsapiens.UCSC.hg19)
    data(gaps.hg19, package="PlasmaTools", envir=environment())
...
## Call local modes in the fragment density.
# callLocalModes <- function(fragments) {
#     fragdt <- fragments[w >= 100 & <= 200]
# }
